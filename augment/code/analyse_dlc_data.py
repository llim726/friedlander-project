...
a_mean = [np.mean(torso_a) for t in trc_data[:,1]]
b_mean = [np.mean(torso_b) for t in trc_data[:,1]]
c_mean = [np.mean(torso_c) for t in trc_data[:,1]]
torso_means = np.array((a_mean[0], b_mean[0], c_mean[0]))
a_std = np.sqrt(np.mean((torso_a - a_mean[0])**2))
b_std = np.sqrt(np.mean((torso_b - b_mean[0])**2))
c_std = np.sqrt(np.mean((torso_c - c_mean[0])**2))
torso_std = np.array((a_std,b_std,c_std))

# torso segements
fig, ax = plt.subplots(figsize=(10,5))

ax.scatter(trc_data[:,1], torso_a, marker='.', label='torso_a = torso_r - torso_l')
ax.plot(trc_data[:,1], a_mean, linestyle='--', c='#FF8C00')
ax.scatter(trc_data[:,1], torso_b, marker='.', label='torso_b = torso_r - xiphoid_process')
ax.plot(trc_data[:,1], b_mean, linestyle='--', c='b')
ax.scatter(trc_data[:,1], torso_c, marker='.', label='torso_c = torso_l - xiphoid_process')
ax.plot(trc_data[:,1], c_mean, linestyle='--', c='r')
...
for i in segments_exclu_torso_headers:
    ...
    y_mean = [np.mean(segment_lengths_dict[i]) for t in trc_data[:,1]]
    y_mean_exclu_torso.append(y_mean[0])
    std_exclu_torso.append(np.sqrt(np.mean((segment_lengths_dict[i] - y_mean)**2)))
    
    ax.scatter(trc_data[:,1], segment_lengths_dict[i], marker='.', label='%s' %i)
    ax.plot(trc_data[:,1], y_mean, label='mean', linestyle='--', c='#FF8C00')
